...
class TestCalculateStudentAverages(unittest.TestCase):
    ...
    def test_GIVEN_student_with_grade_greater_than_10_WHEN_calculated_THEN_raise_ValueError(
        self,
    ):
        students_list = [
            {"nome": "Mario", "cognome": "Rossi", "classe": "3A", "voti": [8, 11, 7]},
        ]

        with self.assertRaises(ValueError):
            calculate_student_averages(students_list)

    def test_GIVEN_student_with_mixed_valid_and_invalid_grades_WHEN_calculated_THEN_raise_ValueError(
        self,
    ):
        students_list = [
            {"nome": "Mario", "cognome": "Rossi", "classe": "3A", "voti": [8, 11, 7]},
